...
class UserDeletePostHandler(val projectsStorage: ProjectsStorage, val userStorage: UserStorage): HttpHandler {
    override fun invoke(request: Request): Response {
        ...
        if(check != null){
            userStorage.userDelete(user)
            return Response(Status.FOUND).header("Location", "/users")
        }
        return Response(Status.FOUND).header("Location", "/users/$idUser/delete")
    }
}